<?php
	...
	function recupAllVoitureDeUtilisateur($idUSelectionner){
		require("modele/connectBD.php");
		$sql="SELECT f.idVehic,v.caract,f.dateD,f.dateF,f.valeur FROM facture as f, vehicule as v  WHERE idUtil=:idu AND v.idVehic=f.idVehic";
		try {
			$commande = $pdo->prepare($sql);
	...
	function vehiculeDejaEnLocation($idv){
		...
		$dayDate=date("Y-m-d");
		$sql="SELECT dateD,dateF FROM facture WHERE idVehic=:idv AND dateD BETWEEN :dayDate AND :dayDate OR dateF BETWEEN :dayDate AND :dayDate AND :dayDate<dateD AND :dayDate>dateF";		try {
			$commande = $pdo->prepare($sql);
			$commande->bindParam(':idv', $idv);
